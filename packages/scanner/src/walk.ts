...
export function collectFiles(roots: string[], ignore?: IgnoreMatcher): string[] {
  ...
  for (const root of roots) walk(root, files, cwd, ignore);
  return files;
}

function walk(path: string, out: string[], cwd: string, ignore?: IgnoreMatcher): void {
  let st;
  try {
    st = statSync(path);
  } catch {
  ...
  if (st.isDirectory()) {
    for (const entry of readdirSync(path)) {
      ...
      walk(join(path, entry), out, cwd, ignore);
    }
  } else if (st.isFile()) {
