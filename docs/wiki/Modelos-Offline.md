...
## Triagem em lote (LLM local)

```bash
# Heurística (sem GPU):
npm run triage:offline -- --sarif codehero.sarif --out reports/triage.json

# Com endpoint OpenAI-compatible (Ollama / llama.cpp / modelo local):
npm run triage:offline -- --sarif codehero.sarif --out reports/triage.json \
...
Workflow exemplo (schedule/manual): `examples/github-workflows/offline-triage.example.yml`.

Ingest também aceita `properties.triageScore` no SARIF, se a triagem for fundida antes do upload.

## Orquestração de agentes / ruleforge / CVE
...
| Lote diário (agentes) | `ruleforgeDaily` / Esteira no portal | propõe; humano aprova; evolve decide |
...
- Não chamar LLM de triagem por arquivo no GitHub Action do PR.
- Não promover regra proposta por agentes sem corpus.
